	...
	for k in dict_sizes:
		...
		for i in range(0,len(ids)):
			if(ids[i][0] == k):
				...
				#ON PC
				#dataThread.client.publish(f"data/{ids[i][0]}_{j}", payload=payload_json, qos=0, retain=False)
				#ON RPI
...
'''
'''
#calib près de la camera le 17/01/2022
...
if __name__ == '__main__':
	...
	#for imgage in range(4,5):
		aruco_dict = aruco.Dictionary_get(aruco.DICT_4X4_100 )
		parameters =  aruco.DetectorParameters_create()
		M_Calib = None
		frame=frame_pi.array
		#str = f'./Images_Tests/image{imgage}.jpg'
		...
		if ids is not None:
			...
			for i in range(0,len(ids)):
				...
				if(ids[i][0] in dict_sizes):
					...
					#np.eye(3), D=np.zeros((1,5))
					#rvec , tvec, _ = aruco.estimatePoseSingleMarkers(corners[i], markerSizeInCM, np.eye(3), np.zeros((1,5)) )
