<<<<<<< Updated upstream:BigBot/Robot.py
from enum import IntEnum
from motorLogic import goForward
=======
>>>>>>> Stashed changes:BigBot/Anthony/Robot.py
import serial
import math
import time
import sys
from utils import computeDict
	...
	def debugIndivMotors(self):
		...
		while(True):
			for motor in self.Emotor:
				print(motor)
				self.set_motor(motor,self.BACKWARD,self.ON)
				self.serialWriteReg()
				time.sleep(1)
				self.set_motor(motor,self.FORWARD,self.ON)
				self.serialWriteReg()
				time.sleep(1)
